...
#Considerando count com condição
cliques_por_busca_diaria = buscas %>% 
    ...
    group_by(dia, group) %>%
    summarise(qt_searchs = n(), 
              qt_clicks = nrow(subset(cliques_por_busca_diaria, num_clicks != 0)),
              taxa_clique = (qt_clicks/qt_searchs)*100,
              qt_no_clicks = sum(num_clicks == 0)) %>%
    ggplot(aes(x = dia , y = taxa_clique, fill = group)) +
    geom_col(position = "dodge") +
...
```{r}
# Usar percentil para ver o que as primeiras 25% costumam clicar, ver a minoria tambem
primeiros_cliques = buscas %>% 
    filter(results > 0, !is.na(first_click), first_click <= 100) %>%
    mutate(dia = floor_date(session_start_date, unit = "day")) %>%
    ggplot(aes(y = first_click, x = dia, color = group)) + 
    geom_jitter(alpha = .8) +
    ...
    labs(x = "Dia", y = "Proporção dos primeiros cliques", 
         title = "Distribuição dos primeiros cliques dos usuários")# +
   # scale_x_date(breaks = primeiros_cliques$dia)
...
## 3. Qual é a nossa taxa de resultados zero no geral? Como isso varia entre os grupos?
    ...
    Taxa de zero results (resultados zero): a proporção de pesquisas que retornaram nenhum resultado.
...
```
    ...
    Para analisar a taxa de clique diária, vamos considerar a razão entre a quantidade dos resultados que retornaram zero e todos os resultados retornados pela pesquisa.
    
```{r}

taxa_resultados_zero = buscas %>% 
    select(session_id, num_clicks, search_index, session_start_date, group, results) %>% 
    mutate(dia = floor_date(session_start_date, unit = "day")) %>% 
    group_by(group) %>%
    summarise(zero = sum(results == 0), total = n()) %>% 
    mutate(taxa_zero = zero/total * 100)# %>%
   # ggplot(aes(group, taxa_zero)) +
    #geom_bar(aes(fill = group), stat = "identity", position = "dodge")
taxa_resultados_zero
...
taxa_resultados_zero = buscas %>% 
    ...
    summarise(zero = sum(results == 0), total = n()) %>% 
    mutate(taxa_zero = zero/total * 100) %>% 
    ggplot(aes(y = taxa_zero, x = dia)) + 
    geom_col(position = "dodge") +
    geom_rug(alpha = .2, color = "red") + 
    geom_hline(aes(yintercept = mean(taxa_zero), linetype="Mean"), colour="#BB0000") +
    geom_text(aes(x = dia, y = taxa_zero, label=paste(trunc(taxa_zero), "%")), vjust=1.6, color="white", size=3.5)
    labs(x = "Dia", y = "Taxa de Resultados Zero (%)", 
         title = "Distribuição taxa de resultados zero no geral") 
...
taxa_resultados_zero = buscas %>% 
   ...
   # mutate(dia = floor_date(session_start_date, unit = "day")) %>% 
    ...
    summarise(zero = sum(results == 0), total = n()) %>% 
    mutate(taxa_zero = zero/total * 100) %>% 
    ggplot(aes(y = taxa_zero, x = group, fill = group)) + 
    geom_col(position = "dodge") +
    geom_rug(alpha = .2, color = "red") + 
    geom_hline(aes(yintercept = mean(taxa_zero), linetype="Mean"), colour="#BB0000") +
    geom_text(aes(x = group, y = taxa_zero, label=paste(trunc(taxa_zero), "%")), vjust=1.6, color="white", size=3.5)
    labs(x = "Dia", y = "Taxa de Resultados Zero (%)", 
         title = "Distribuição taxa de resultados zero no geral") 
...
taxa_resultados_zero = buscas %>% 
    ...
    ggplot(aes(y = taxa_zero, x = dia, fill = group)) + 
    geom_col(position = "dodge") +
    geom_rug(alpha = .2, color = "red") + 
    geom_smooth(aes(x = dia , y = taxa_zero), method = lm, se = FALSE) +
    geom_hline(aes(yintercept = mean(taxa_zero), linetype="Mean"), colour="#BB0000") +
    geom_text(aes(x = dia, y = taxa_zero, label=paste(trunc(taxa_zero), "%")), vjust=1.6, color="black", size=3.5)
    labs(x = "Dia", y = "Taxa de Resultados Zero (%)", 
         title = "Distribuição taxa de resultados zero no geral") 
...
taxa_resultados_zero = buscas %>% 
    ...
    ggplot(aes(y = taxa_zero, x = dia, color = group)) + 
    geom_point() +
    geom_line() +
    geom_rug(alpha = .2, color = "red") + 
    geom_hline(aes(yintercept = mean(taxa_zero), linetype="Mean"), colour="#BB0000") +
